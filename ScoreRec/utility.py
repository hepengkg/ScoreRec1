...
def extract_axis_1(data, indices):
    ...
    res = torch.stack(res, dim=0).unsqueeze(1) #128 1 64
    return res

def calculate_hit_mmr(sorted_list,topk,true_items,hit_purchase,ndcg_purchase,mrr_purchase):
...
def calculate_hit(sorted_list,topk,true_items,hit_purchase,ndcg_purchase):
    for i in range(len(topk)):
        ...
        for j in range(len(true_items)):
            ...
            # def calculate_hit_zero(sorted_list,topk,true_items,hit_purchase,ndcg_purchase):
#     for i in range(len(topk)):
#         rec_list = sorted_list[:, -topk[i]:]
#         # print(rec_list)
#         # print(true_items)
...
def calculate_hit_zero(sorted_list, topk, true_items, recall_purchase, ndcg_purchase):
    for i in range(len(topk)):
        rec_list = sorted_list[:, -topk[i]:]
        for j in range(len(true_items)):
            if true_items[j] in rec_list[j]:
                rank = topk[i] - np.argwhere(rec_list[j] == true_items[j])[0][0]
                recall_purchase[i] += 1.0
                ndcg_purchase[i] += 1.0 / np.log2(rank + 1)


def calculate_hit_fre(sorted_list, topk, true_items, hit_purchase, ndcg_purchase,hit_30, hit_20, hit_15,hit_10, hit_5,hit_1,data_dict):
    for i in range(len(topk)):
        rec_list = sorted_list[:, -topk[i]:]
        for j in range(len(true_items)):
            if true_items[j] in rec_list[j]:
                ...
                # 检查 true_items[j] 是否在 data_dict 的特定百分比中
                if true_items[j] in data_dict["30%"]:
                    hit_30[i] += 1.0
                if true_items[j] in data_dict["20%"]:
                    hit_20[i] += 1.0
                if true_items[j] in data_dict["10%"]:
                    hit_10[i] += 1.0
                if true_items[j] in data_dict["15%"]:
                    hit_15[i] += 1.0
                if true_items[j] in data_dict["5%"]:
                    hit_5[i] += 1.0
                if true_items[j] in data_dict["1%"]:
                    hit_1[i] += 1.0

def gini_coefficient(user_lists, num_items):
    """
    Calculate the Gini coefficient of the recommendation result
    Args:
        user_lists: List[List[int]], recommended list of users (each sub-list is item ID)
        num_items: int indicates the total number of items
    Returns:
        float: Gini coefficient (range [0,1])
    """
    counts = np.zeros(num_items, dtype=int)
    for r_u in user_lists:
        for item in r_u:
            counts[item] += 1
