...
def parse_args():
    parser = argparse.ArgumentParser(description="DreamRec baselines.")

    parser.add_argument('--epoch', type=int, default=500,
                        help='Number of max epochs.')
    parser.add_argument('--Model', type=str, default='DreamRec',
                        help='model name.')
...
class diffusion():
    ...
    def p_losses(self, denoise_model, x_start, h, t, noise=None, loss_type="l2"): #修改
        # 
        if noise is None:
            noise = torch.randn_like(x_start) 

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)   # 正向加噪
    ...
    def p_sample(self, model_forward, model_forward_uncon, x, h, t, t_index):
        ...
        x_start = (1 + self.w) * model_forward(x, h, t) - self.w * model_forward_uncon(x, t)#100,64
        x_t = x #100,64  噪声
        model_mean = (
        ...
        if t_index == 0:
            ...
            posterior_variance_t = extract(self.posterior_variance, t, x.shape)#100,1
            noise = torch.randn_like(x)#100,64

            return model_mean + torch.sqrt(posterior_variance_t) * noise 
        
    @torch.no_grad()
    def sample(self, model_forward, model_forward_uncon, h):
        x = torch.randn_like(h) #100,64
        for n in reversed(range(0, self.timesteps)): #199 198 ...
...
class Tenc(nn.Module):
    ...
    def predict(self, states, len_states, diff):
        ...
        ff_out = self.ln_3(ff_out)
        state_hidden = extract_axis_1(ff_out, len_states - 1)
        h = state_hidden.squeeze()#100,64
        x = diff.sample(self.forward, self.forward_uncon, h)#100,64
        test_item_emb = self.item_embeddings.weight
        scores = torch.matmul(x, test_item_emb.transpose(0, 1))

        return scores,test_item_emb

def evaluate(model, test_data, diff, device,writer,VAL,itr):
    ...
    for i in range(num_total // batch_size):
        ...
        states = np.array(seq_b) #100 10
        states = torch.LongTensor(states) #100 10
        states = states.to(device)

        prediction,test_item_emb = model.predict(states, np.array(len_seq_b), diff)
...
if __name__ == '__main__':
    ...
    model = Tenc(args.hidden_factor,item_num, seq_size, args.dropout_rate, args.diffuser_type, device) #去噪模块
    ...
    for i in range(args.epoch):
        ...
        for j in range(num_batches):
            ...
            loss, predicted_x = diff.p_losses(model, x_start, h, n, loss_type='l2') #修改
        ...
        if args.report_epoch:
            ...
            if (i + 1) % 1 == 0:
                ...
                print('-------------------------- VAL PHRASE --------------------------')
                VAL = True
                _ = evaluate(model, 'val_data.df', diff, device,writer,VAL,i)
                print('-------------------------- TEST PHRASE -------------------------')
                VAL = False
                _ = evaluate(model, 'test_data.df', diff, device,writer,VAL,i)
                time = Time.strftime("%H: %M: %S", Time.gmtime(Time.time() - eval_start))
                print("Evalution cost: " + time)
